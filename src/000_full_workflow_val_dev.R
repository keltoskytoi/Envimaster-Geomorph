...
demval <- raster::raster(file.path(envrmt$path_001_org, "dem_val_ex.tif")) # use dem of aoi
...
stckmof <- LEGION_dem_v2(dem    =demmof,
                      tmp    =envrmt$path_tmp,
                      proj   =utm,
                      filter =fs)

stckval <- LEGION_dem_v2(dem    =demval,
                         tmp    =envrmt$path_tmp,
                         proj   =utm,
                         filter =fs)
...
cenith_fillsinks(dem=demmof,
                 output=envrmt$path_002_processed,
                 tmp=envrmt$path_tmp,
                 0,
                 proj=utm)

sommof <- raster::raster(file.path(envrmt$path_002_processed, "som.tif")) 

cenith_fillsinks(dem=demval,
                 output=envrmt$path_002_processed,
                 tmp=envrmt$path_tmp,
                 0,
                 proj=utm)

somval <- raster::raster(file.path(envrmt$path_002_processed, "som.tif")) 
...
seg <- Cenith(chm=som,a=1,b=7,h=0.4)
segmof <- Cenith(chm=sommof,a=1,b=7,h=0.4)
segval <- Cenith(chm=somval,a=1,b=7,h=0.4)
# view result polygone
mapview::mapview(som)+seg$polygon
mapview::mapview(sommof)+segmof$polygon
mapview::mapview(somval)+segval$polygon
#write out polygones
