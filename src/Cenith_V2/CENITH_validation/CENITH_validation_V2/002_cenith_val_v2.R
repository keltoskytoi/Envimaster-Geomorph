...
for (c in seq(1:length(h))){
  ...
  cat       ("#############################",sep="\n")
  if(c==1){
    res <-cenith_val_a(chm,a,b,h[c],vp)
  }    else {
    res2 <-cenith_val_a(chm,a,b,h[c],vp)
    res= rbind(res,res2)}
  cat       ("#############################",sep="\n")
  cat       ("#############################",sep="\n")
  cat(paste0("### Cenith rdy with h ",as.factor(c)," / ",as.factor(length(h))," ###",sep = "\n"))
  cat       ("#############################",sep="\n")
  cat       ("#############################",sep="\n")
  
  
}
  names(res)<- c("a","b","height","hit","miss","empty")
  cat       ("################################",sep="\n")
...
#'cval <- cenith_val_v2(chm,f=1,c(0.04,0.08),c(0.1),h=c(8,13),vp=vp)
#'cval
