...
#need a specific SAGA Version
#build a function to check and disply the current versions 
mtpi <- function(dem,output,tmp,proj) {
  raster::writeRaster(dem,filename=paste0(file.path(tmp),"/dem.sdat"),overwrite = TRUE,NAflag = 0)
  RSAGA::rsaga.geoprocessor(lib = "ta_morphometry", module = 24, # unable to find inhered module nr
  ...
  pr4 <- proj
  tpi <- raster::raster(file.path(tmp, "mtpi.sdat"))
  proj4string(tpi) <- pr4
  raster::writeRaster(tpi,filename=paste0(file.path(output),"/mtpi.tif"),overwrite = TRUE,NAflag = 0)
}
