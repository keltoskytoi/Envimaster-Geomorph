...
#Note v1_2: add filter function and return only 1 stack
LEGION_dem3 <- function(dem,tmp,method=6,units=0,radius=100,proj,filter){
...
# brick all layers and return brick, 
    ...
    bric <- raster::brick(slo,asp,gen,pro,pla,tan,lon,cro,min,max,tol,rot,vis,svf,sim,ter,dis)
    #if (f==1){
    #  st <-bric
    #}else{
    #  st <-stack(st,bric)
    #}
    #st <-stack(bric)
    #cat(" before return but in lappyl",sep = "\n")
    #print(bric[[1]])
    return(bric)
  })
  cat("end of lapply ",sep = "\n")
